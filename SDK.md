...
| Error locality (worst-fraction share, chi-squared null, per-cell enrichment) | `holo/locality.py` | Wilson's Creek 91% against the cannon's 16% in the worst 1% at similar headline error (`results/gpu_sweep.md`); chi-squared_1 null 8.45% at f=1%, measured 8.51% at n=50k (`tests/test_locality.py`); `docs/locality.md` |
| Resonator factorization (identity and position codebooks) | `holo/resonator.py` | 100% single-object and 84.67% three-object recovery at d=4096 over 50 seeded trials; capacity cliff (`out/resonator_cliff.png`, `bench/resonator_sweep.py`); stable small-grid tests (`tests/test_resonator.py`); negative on real captures (`results/resonator_capture.md`); `docs/resonator.md` |
...
- **5090 productionization** (box session, published over the GPU box
