...
wmag = np.linalg.norm(freqs, axis=1)
wmax = wmag.max()
print(
    "|w| quantiles 10/50/90:",
    np.percentile(wmag, [10, 50, 90]).round(1),
    "max",
    wmax.round(1),
)
F = {}
for n in (
    ...
    F[n] = place.fingerprint(scene, freqs, sigma)
    uni = SplatScene(
        rng.uniform(0, 1, scene.mu.shape).astype(np.float32),
        scene.cov.copy(),
        scene.amp.copy(),
    )
...
for n, f in F.items():
    ph = np.angle(f)
    live = np.abs(f) > 1e-3 * np.abs(f).max()
    print(
        "%-26s near-real (|phase|<pi/4) %.2f  live %.2f  "
        "|F| decay: median|F| at |w|<q50 / >q50 = %.1f"
        % (
            n,
            np.mean(np.abs(ph[live]) < np.pi / 4),
            live.mean(),
            np.median(np.abs(f)[wmag < np.median(wmag)])
            / max(np.median(np.abs(f)[wmag >= np.median(wmag)]), 1e-12),
        ),
        flush=True,
    )
pairs = [
    ...
    ("uniform-wilsons-creek", "uniform-redrock"),
]
for frac in (0.0, 0.2, 0.4, 0.6):
    ...
    print(
        "drop |w|<%.1f max kept=%5d | " % (frac, keep.sum())
        + " ".join(
            "%s-%s %.2f/%.2f" % (a[:7], b[:7], sc(F[a], F[b], 0), sc(F[a], F[b], 1))
            for a, b in pairs
        ),
        flush=True,
    )
