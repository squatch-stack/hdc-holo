...
for n in names:
    for w in (0.0, 1.0):
        out[f"{n} vs own-uniform w={w}"] = place.correlate(
            fps[n], ufps[n], freqs, grid, w
        )[0]
for a, b in [
    ("wilsons-creek", "redrock"),
    ("wilsons-creek", "research-library"),
    ("cannon", "brookline-station"),
    ("wilsons-creek", "cannon"),
]:
    for w in (0.0, 1.0):
        out[f"{a} vs {b} w={w}"] = place.correlate(fps[a], fps[b], freqs, grid, w)[0]
        out[f"uniform({a}) vs uniform({b}) w={w}"] = place.correlate(
            ufps[a], ufps[b], freqs, grid, w
        )[0]
for k, v in out.items():
    print(f"{v:6.3f}  {k}")
with open("uniform_test.json", "w") as fh:
