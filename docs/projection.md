# Analytic L2 projection of spatial cells

*[← docs index](README.md) · [ridge fitting](fit.md)*

`project_cells` replaces forward importance weighting with a windowed Gram
solve, returning the same `{band: {cell: (C, d)}}` complex64 bundles as
`encode_bands`. It is opt-in; codecs, replication and decoders are unchanged.

The default eps is **1e-3** (`project.default_eps`). This is an absolute
choice of threshold on the **relative eigenvalue magnitudes**:
`abs(eigenvalue) > eps * max(abs(eigenvalues))`. It is not an absolute
threshold in the unnormalized Gram's physical units, nor a rank fraction.
The default norm limit is **1.2** (`project.default_limit`); the measured
knee bracket is **1.15 to 1.2** (`project.knee_bracket`). These are the
decided settings, not a new sweep or an automatic tuning procedure.

```python
from holo import band_codebooks, build_scene, encode_bands, project_cells
import numpy as np

scene, scales, box = build_scene(scene_path)  # caller-supplied path
books = band_codebooks(np.random.default_rng(42))
forward, members = encode_bands(scene, scales, books)
solved = project_cells(scene, members, books)
```

```python
project_cells(scene, members, books, bands=None,
              setting=("eps", 1e-3), window=0.5,
              limit=1.2, allow_divergence=False)
```

Pass the same band descriptors that produced the members and books.
The window standard deviation is `window * cell / 2`. One Gram and one
`eigh` serve each populated band; cell-centre phases restore world coordinates.
The windowed right-hand side dispatches through `spectral_bundle`.
`holo.fit.project_cells` is also exported.

The gate raises `holo.projection.Diverged` before returning any result.
It compares median solved norms to median forward norms over the first
64 cells in membership order, preserving the driver's reference selection.
Before gating, it scores forward density against `exact_slice` on top-down
and side mass-mode planes, at the original relative pixel spacing. These
use the enclosing integer box (the unit box for normalized captures).
Bands below `SIGNAL_FLOOR` relative to total truth have absent scores.
A band is excluded if it has no signal or its forward relative error is
at least one on any scored plane. In particular, Red Rock `coarse` already
has forward error **1.4294**, worse than zeros
(`project.redrock_coarse_unreconstructed`); exclusion prevents that
pre-existing failure from being read as projection damage. Excluded bands
are still projected and returned; exclusion does not certify their accuracy.
These probe planes and sampled norms are a detector, not a bound at every
point or cell. `allow_divergence=True` deliberately bypasses refusal and
prints the ratio; legacy reproduction requires this explicit override.

Failure modes and cost:

- `keep=0.70` is **37x worse** than not projecting at all, silent in the
  decode; the norm ratio is the signal. See the registered cliff evidence below.
- A shared rank fraction regularises the sparse band least: one setting
  spans **7e8** in actual regularisation (`project.keep_band_spread`).
- Tikhonov's lambda is scene-dependent by five orders of magnitude;
  `1e-6` on saguaro produces a **−7.9%** regression on the dense scene.
  This is why the default is a spectral threshold rather than ridge.
- Historical CPU projection costs span **250–2,900 s** per capture against
  **51–100 s** forward, with **7.6 GB** peak. `require_headroom` runs before
  Gram allocation, accounting for dimension and bundle storage. The new
  forward-referee pass also costs time; these historical timings are not
  measurements of the promoted wrapper.
- **Three captures is three.** The setting passes the recorded gate on all
  three, with no meaningful band degradation; the worst recorded fine-band
  change is −1.0%, not a claim of strict monotonic improvement.
- Composition with adaptive cells is **unmeasured**. The Gram depends on
  cell size, so an adaptive lattice multiplies per-band `eigh` by the number
  of levels present. This remains open; pass a fixed lattice here.

Evidence: `tests/test_projection.py` retains the original Gram/operator and
per-band gate checks, and tests the promoted API, threshold spectrum,
forward-failure exclusion and sparse synthetic accuracy. Historical
`project.*` claims are reused from `claims/registry.jsonl`, not duplicated.

The historical catastrophe calibration had everything broken at or above
97, an 18x gap (`project.divergence_broken_ratio`). That calibration missed
minority-band degradation, which motivated the tighter promoted limit.

## Recorded experiments (historical settings)

In the experiment narrative below, "shipped keep=0.25" and "limit 20"
refer to the former driver configuration, not the promoted API defaults.
The retained tables document both successes and failures.
...
The threshold default and tighter gate are now shipped as an opt-in API.
The norm ratio is empirical; the forward-failure exclusion is now tested.
Three captures is three, and does not establish a corpus guarantee.
...
survive. The opt-in API now lives in `holo/projection.py`, with deterministic
tests and the failure modes documented above.
