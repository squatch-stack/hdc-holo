...
demonstrate the representation on twelve real captures of 142,000 to
1,160,000 splats, evaluated against exact analytic ground truth rather
than against images, and we report the cost plainly: a bundle is roughly
...
orthographic projection, and coordination-free merge become the
\emph{same operation} on the same object, with a measurable budget
attached --- and that the same law marks where the object stops being
useful, which we report as four boundaries rather than as future work.
\end{abstract}
...
experiments, one is measured six ways on real captures, and one is a gap
in the law itself. A representation whose limits are known is more
useful than one whose limits are merely unreported, so \S7 states them
as results and not as apology.

\textbf{Three capabilities.}
...
gap and make hypervector scene state mergeable without coordination.

\textbf{Four boundaries.} Occlusion is outside superposition and no
codebook reaches it (\S7.1). Rotation is outside a \emph{fixed
continuous} codebook, which is a narrower and more actionable statement
than ``rotation is hard'' (\S7.2). A whole-capture bundle localises but
does not recognise, which twelve captures and six diagnostics establish
and which one plausible remedy makes worse (\S7.3). And the capacity law
describes the noise a readout carries rather than whether it is correct,
so every $\sigma$ in this paper is half of a budget (\S6).
...
approximately 400$\times$ larger at complex64 and about 24$\times$
larger at the measured four-bit knee, and 50$\times$ less accurate at
reproducing the field than a current splat codec (\S8). If the task is
...
\textbf{FHRR.} ``Holographic'' here is Plate's --- every item is
distributed across every component of one vector, so any part of the
vector carries a degraded copy of the whole --- and has nothing to do
with display holography or computer-generated holograms, a collision \S9
returns to. A hypervector is $d$ unit-magnitude complex phasors. Binding
...
similarity $0 \pm 1/\sqrt{2d}$, which is the noise floor everything else
trades against.
...
\section{Capability 1 --- A splat scene is one vector}
...
cell-locally. Kernels agree across three compute backends to 2.5e-8. The
same fixed pipeline has since run over all twelve captures of the public
gallery, 142,000 to 1,160,000 splats, without a per-scene setting; \S10
reports what that wider sweep says about the error metric itself.
...
Gaussian mixture evaluated cell-locally. Four further captures --- a
second phone scan, an indoor LiDAR cloud, a Tanks \& Temples scene, and
a stone interior scanned from within its walls (the Brookline
springhouse, whose 15.9\%/28.2\% slices are the best single-axis result
of any capture) --- pass through the same fixed pipeline.}
\label{fig:4}
\end{figure}
...
\section{Capability 2 --- A view is also just a bundle}
...
\section{Capability 3 --- Superposition is almost a CRDT}
...
``decorrelate or denoise'', which we return to in \S10.
...
\textbf{And one thing the law does not describe at all.} The four above
are places the law bends. This one is outside it. $\sigma$ is the noise
a readout carries; it is not whether the readout is \emph{right}. A
decoded codeword must beat every competing codeword in the item memory,
so the same $\sigma$ yields a different error rate depending on how many
competitors there are --- and the law has no term for that count, since
\texttt{R} is 1 for a codeword whether there are two of them or a
hundred. Measured by holding the item count at 64 and varying only the
vocabulary, the dimension needed to answer 90\% of queries correctly
runs 128, 256, 512 for 2, 4 and 19 classes and then saturates through 48
and 106: a fourfold cost in $d$ for a realistic vocabulary, with the
required $\sigma$ halving from 0.50 to 0.25. Every capacity statement in
this paper is therefore a statement about noise, and converting one into
a statement about accuracy needs the vocabulary size as well. We state
this as a gap rather than close it: the missing piece is a standard
decision bound among \texttt{k} alternatives at a given signal-to-noise
ratio, and naming it is what keeps a reader from budgeting a dimension
from $\sigma$ alone, which is what we did before measuring this.
...
one, which is a failure of the law's \emph{scope} that only appeared
when we asked it a question about accuracy that it was never posed to
answer.

\section{What one vector cannot do}

The boundaries below are results in the same sense as \S\S3--5. Two are
arguments: no experiment could have come out differently and none was
needed. One is measured, on the same twelve captures as everything else,
and its most plausible remedy was tried and made matters worse.
...
\subsection{Occlusion is outside superposition}

The hard limit is occlusion, and it is a limit rather than an
engineering gap. Alpha compositing accumulates as \texttt{1 - Π(1 -
α\_i)}, which is neither linear nor order-free, and an additively
encoded bundle carries no ordering to recover --- so no choice of
codebook reaches it, and the representation renders what integrates.
What remains open is narrower than ``occlusion'': whether a
\emph{bounded} surrogate is useful, in the manner of the moment-based
order-independent transparency literature, with a classical compositing
pass over a holographic density. That is a graphics question about
approximation quality, not a question about superposition.

\subsection{Rotation is outside a fixed continuous codebook}
...
frequency row to a different one, so with a \emph{fixed} set of
frequency rows drawn once from a continuous distribution there is no
elementwise operator that rotates the field, and the ramp construction
cannot be recovered by any choice of $d$. What this rules out is a fixed
...
\subsection{A whole-capture bundle localises but does not recognise}

A whole-capture bundle at d = 8,192, blurred to a fortieth of its cube,
does not recognise places. On the twelve gallery captures --- one true
re-capture, two exact crops of parents, three same-class objects as hard
negatives --- phase correlation through the shift
property~\cite{voelker2021ssp} ranked 0 of 6 known partners first, and
the reason was measurable: a mass-centred crop of a capture whose
subject is dense and whose background is a sparse halo puts half the
alpha mass within a tenth of the cube of its centre, and at that
...
object side: with two candidate objects in one cube, every probe --- the
gun's own codeword included --- preferred the denser one, so a prototype
superposed from two cannon instances ``finding'' a third was the
target's mass, not the prototype's class. What survives is sub-map
...
\subsection{And the law describes noise, not accuracy}

The fourth boundary is \S6's fifth entry and is not repeated here. It is
listed among the boundaries because it has the same character as these
three: it is a statement about what the representation and its theory do
not give you, established by measurement, and knowing it changes what a
...
ladder run below the nibble --- on synthetic cells and then on two of
the captures --- located the knee of the trade between dimension and
...
without moving the knee. At that knee a bundle is about 24$\times$ the
SPZ file of the same capture, median over the twelve gallery captures.
The loss is smaller than the table shows and it is still a loss; the
...
Conflict-free replication of \emph{spatial} data is likewise occupied,
and we say so because our own positioning notes claimed otherwise until
we looked: a conflict-free voxel structure for distributed collaborative
...
data'' is false, and the claim \S5 can defend is the narrower one about
...
per dirty cell over a 250$\times$ range, live state flat in the number
of contributions, and the observation that summing peers is the wrong
read rule wherever two contributors saw the same matter, since addition
...
\textbf{Replications and measured comparisons.} Three of the works above
were met with experiments rather than citations. Renner et
al.~\cite{renner2024resonator}'s resonator network, which factorizes a
superposed scene into identity and position by iterating
unbind-project-normalize, replicates on synthetic scenes with this
repository's fractional-power position key: 100\% single-object and
84.7\% three-object recovery at d = 4,096, with a capacity cliff where
the product of codebook sizes exceeds the dimension by an order of
magnitude. On real captures it does not converge, for a reason given in
\S7.3; Yeung et al.~\cite{yeung2026cleanup}'s comparison of cleanup
nonlinearities suggests the choice of projection is where a second
attempt would start, and in-memory factorizers (Langenegger et
al.~\cite{langenegger2023factorizer}) are where its cost would be paid.
HyperSpace~\cite{snyder2026hyperspace}'s finding that HRR needs half the
memory of FHRR is a same-dimension statement, and a phasor carries two
real degrees of freedom; at equal bytes --- FHRR at half the dimension
against HRR at full --- the two recover the same fraction of a bundled
dictionary at every load through 0.2 of nominal capacity on three
dimensions, so the choice between them is about operators, not capacity,
and cleanup dominates the operator table for both. Quantized-phase
FHRR's three-to-four-bit phases sit exactly at the knee \S8 reports. The
matched referee of \S6 has a neural-field counterpart in Spectral
Prefiltering of Neural Fields~\cite{yaldiz2025prefilter} (Yaldiz et
al.), which pre-filters the target rather than the estimate for the same
reason we blur the referee to the pixel footprint. And the phase-only
correlation that scores arrangement in \S7.3 is Knapp and
Carter~\cite{knapp1976gcc}'s PHAT weighting, with the well-known
consequence that the peak becomes as narrow as the highest frequency
present and needs a search grid that can see it.
...
rather than per view. HyperSpace is the nearest VSA-framework effort and
contains no scene, splat, or replication content.

\textbf{Scene memory and shared maps --- where the demand sits.} Two
active lines frame what a fixed-size queryable scene is \emph{for}.
Language-embedded splatting (LangSplat~\cite{qin2023langsplat},
OpenGaussian~\cite{wu2024opengaussian}) attaches per-Gaussian feature
...
size never moves. Embodied-agent memory (Clio~\cite{maggio2024clio}'s
task-driven scene graphs, 3D-Mem~\cite{yang20243dmem}, and lately splat
maps held as persistent spatial memory in GSMem~\cite{lu2026gsmem})
states its own pains plainly: memory that grows with the environment,
retrieval that needs graph or language machinery, and nothing that
merges across agents. Collaborative splat SLAM
(MAC-Ego3D~\cite{xu2024macego3d}, GS-Share~\cite{zhang2025gsshare})
meets the merge problem directly and fights communication load and map
fusion for it; no published system in either line treats the map as a
replicated data type. The first two pains are what \S3 answers --- one
fixed-size vector, queried by unbinding --- and the third is \S5.

\textbf{A note on the word ``holographic.''} In graphics it now denotes
wave-optics display holography --- Gaussian Wave
Splatting~\cite{choi2025gaussianwave} propagates complex-valued
Gaussians for computer-generated holograms. This paper uses the word
only in Plate's sense: interference-style superposition of distributed
representations. The two share complex arithmetic and nothing else.
...
against in \S8 is better at compression. The contribution here is not a
...
\section{Remaining limits and future work}

Storage is large in absolute terms, as \S8 quantifies. Determinism is
semantic rather than bitwise --- recomputed vectors agree to about a
part in 10$^7$, far under any decision threshold, but digests must hash
transmitted bytes rather than recomputed sums. Banded and clustered
readout strategies pay off only when the data has structure to exploit:
spatial locality for scenes, and its analogue elsewhere.

\textbf{A note on what ``fixed size'' is worth.} Fixed size is a
property of the representation, not an argument that it stores any
particular thing more cheaply. Measured at object granularity against an
exact table, a bundle's advantage at equal accuracy is a bounded factor
rather than an order of magnitude, it exists only at \S8's four-bit rate
point, and \S6's vocabulary cost works against it. We note this because
the fixed- size property is easy to over-read; it bears on none of \S3's
algebraic queries, which are what the representation is for.

\textbf{A note on the referee.} The relative-L2 error this paper reports
is two different quantities depending on the scene: on some captures it
measures reconstruction fidelity, on others the worst one percent of
pixels carries nearly all of the squared error and the number is a spike
detector. The share of squared error in the worst fraction of pixels,
against the χ$^2$ reference an iid-Gaussian residual would give,
separates the two readings, and every headline error in \S6 has been
re-read under it.
...
the accidental shrinkage observed in \S8 suggests that deliberate
...
And \S6's fifth entry --- that the law describes noise rather than
accuracy --- wants closing properly: a decision bound among \texttt{k}
alternatives would turn every $\sigma$ in this paper into a budget a
reader could act on, and we currently hand them half of one.
...
storage than a modern splat codec at complex64 and about 24$\times$ at
the measured four-bit knee, at fifty times the error, for a
...
Tony A. Plate.
\newblock Holographic Reduced Representation: Distributed Representation
...
Dominic Maggio and Yun Chang and Nathan Hughes and Matthew Trang and Dan
Griffith and Carlyn Dougherty and Eric Cristofalo and Lukas Schmid and
Luca Carlone.
\newblock Clio: Real-time Task-Driven Open-Set 3D Scene Graphs.
, 2024.
\newblock arXiv:2404.13696.

\bibitem{yang20243dmem}
Yuncong Yang and Han Yang and Jiachen Zhou and Peihao Chen and Hongxin
Zhang and Yilun Du and Chuang Gan.
\newblock 3D-Mem: 3D Scene Memory for Embodied Exploration and
Reasoning.
, 2024.
\newblock arXiv:2411.17735.

\bibitem{wu2024opengaussian}
Yanmin Wu and Jiarui Meng and Haijie Li and Chenming Wu and Yahao Shi
and Xinhua Cheng and Chen Zhao and Haocheng Feng and Errui Ding and
Jingdong Wang and Jian Zhang.
\newblock OpenGaussian: Towards Point-Level 3D Gaussian-based Open
Vocabulary Understanding.
, 2024.
\newblock arXiv:2406.02058.

\bibitem{lu2026gsmem}
Yiren Lu and Yi Du and Disheng Liu and Yunlai Zhou and Chen Wang and Yu
Yin.
\newblock GSMem: 3D Gaussian Splatting as Persistent Spatial Memory for
Zero-Shot Embodied Exploration and Reasoning.
, 2026.
\newblock arXiv:2603.19137.

\bibitem{zhang2025gsshare}
Xinran Zhang and Hanqi Zhu and Yifan Duan and Yanyong Zhang.
\newblock GS-Share: Enabling High-fidelity Map Sharing with Incremental
Gaussian Splatting.
, 2025.
\newblock arXiv:2510.02884.

\bibitem{qin2023langsplat}
Minghan Qin and Wanhua Li and Jiawei Zhou and Haoqian Wang and Hanspeter
Pfister.
\newblock LangSplat: 3D Language Gaussian Splatting.
, 2023.
\newblock arXiv:2312.16084.

\bibitem{choi2025gaussianwave}
Suyeon Choi and Brian Chao and Jacqueline Yang and Manu Gopakumar and
Gordon Wetzstein.
\newblock Gaussian Wave Splatting for Computer-Generated Holography.
, 2025.
\newblock arXiv:2505.06582.

\bibitem{xu2024macego3d}
Xiaohao Xu and Feng Xue and Shibo Zhao and Yike Pan and Sebastian
Scherer and Xiaonan Huang.
\newblock MAC-Ego3D: Multi-Agent Gaussian Consensus for Real-Time
Collaborative Ego-Motion and Photorealistic 3D Reconstruction.
, 2024.
\newblock arXiv:2412.09723.

\bibitem{renner2024resonator}
Alpha Renner and Lazar Supic and Andreea Danielescu and Giacomo Indiveri
and Bruno A. Olshausen and Yulia Sandamirskaya and Friedrich T. Sommer
and E. Paxon Frady.
\newblock Neuromorphic visual scene understanding with resonator
networks.
\newblock \emph{Nature Machine Intelligence} \textbf{6}, 2024.
\newblock arXiv:2208.12880.

\bibitem{yeung2026cleanup}
Calvin Yeung and Prathyush Poduval and Mohsen Imani.
\newblock A comparative study of nonlinear cleanup rules in resonator
networks.
\newblock \emph{Frontiers in Artificial Intelligence} \textbf{9}, 2026.
\newblock \texttt{doi:10.3389/frai.2026.1793314}.

\bibitem{langenegger2023factorizer}
Jovin Langenegger and Geethan Karunaratne and Michael Hersche and Luca
Benini and Abu Sebastian and Abbas Rahimi.
\newblock In-memory factorization of holographic perceptual
representations.
\newblock \emph{Nature Nanotechnology}, 2023.
\newblock arXiv:2211.05052.
\newblock \texttt{doi:10.1038/s41565-023-01357-8}.

\bibitem{yaldiz2025prefilter}
Mustafa B. Yaldiz and Ishit Mehta and Nithin Raghavan and Andreas
Meuleman and Tzu-Mao Li and Ravi Ramamoorthi.
\newblock Spectral Prefiltering of Neural Fields.
\newblock \emph{arXiv:2510.08394}, 2025.

\bibitem{knapp1976gcc}
Charles H. Knapp and G. Clifford Carter.
\newblock The generalized correlation method for estimation of time
delay.
\newblock \emph{IEEE Transactions on Acoustics, Speech, and Signal
Processing} \textbf{24}(4):320--327, 1976.
