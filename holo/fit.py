...
class HoloRegressor:
    ...
    def fit(self, points, values, lam=1e-3):
        ...
        # 2026-09-04 on numpy 2.5.2: 3 non-finite results in 12,000 random
        # products across 10 processes, all in one process, none on rerun
        # with the same seed — rare, heap-dependent, and still there. OpenBLAS/MKL wheels (Linux) are clean and run
        # numpy 2. If you ever lift the macOS cap, run the suite several
